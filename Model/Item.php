...
class Model_Item extends Model_Core_Table
{
	...
	{
		$query = "SELECT * FROM `eav_attribute` WHERE `entity_type_id` = 7 AND `status` = '".self::STATUS_ACTIVE."'";
		return Ccc::getModel('Eav_Attribute')->fetchAll($query);
	}

	public function getAttributeValue($attribute)
	{
		$query = "SELECT `value` FROM `item_{$attribute->backend_type}` WHERE `attribute_id` = '{$attribute->getId()}' AND `entity_id` = '{$this->getId()}' ";
